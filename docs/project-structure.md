...
- [`navigation/`](/src/app/navigation) — `RootStack`, `HomeTabStack`, `DrawerNavigator`, `ToolsScreen`, `ContactsTabScreen` (composes the contacts list and map behind a List | Map switch).
...
- [`map/`](/src/features/map) — **The Contacts tab's Map view.** Renders contacts as map markers (colored by staleness), a swipable carousel of contact cards, the map-onboarding sheet, the color-key legend, and share-address. _Contains: screens, components, lib, types._
