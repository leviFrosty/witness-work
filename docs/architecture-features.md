...
- It's consumed by another **shared** module — since `shared → feature` is forbidden, the dependency forces it into `shared/`. (Example: shared stores and types that are read by multiple feature consumers.)
...
14. `refactor(features): drain features/home and extract a profile feature` (ContributionGraph + ProfileCard + ProfileDetailOverlay + card interaction primitives + lib/profileStats + hooks/useDailyMinutes → new `features/profile/`; `home/lib/supporterNudge` → `supporter/lib/`; `home/components/BackupReminder` → `settings/components/`; `components/WeekStripTeaser` → `service-reports/components/`. After this commit `features/home/` only contains `screens/HomeScreen.tsx`.)
