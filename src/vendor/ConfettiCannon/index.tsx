...
class Explosion extends React.PureComponent<ExplosionProps, State> {
  state: State = {
    items: [],
  }
  ...
  getItems = (prevColors: string[]): Item[] => {
    const { count, colors = DEFAULT_COLORS } = this.props
    const { items } = this.state

    const difference = items.length < count ? count - items.length : 0

    const newItems: Item[] = Array(difference)
      .fill(null)
      .map(
        (): Item => ({
  ...
  start = (resume: boolean = false) => {
    ...
    this.sequence &&
      this.sequence.start(({ finished }: { finished: boolean }) => {
        if (finished) {
      ...
      <React.Fragment>
        {items.map((item: Item, index: number) => {
          ...
          const transform: Array<{
            ...
            rotate?: Animated.AnimatedInterpolation<number | string>
            rotateX?: Animated.AnimatedInterpolation<number | string>
            rotateY?: Animated.AnimatedInterpolation<number | string>
            perspective?: number
