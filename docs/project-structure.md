...
- [`contacts/`](/src/features/contacts) — **People the user is ministering to.** CRUD for contact records (name, address, geocoded pin, custom fields, personal notes), the contacts list with sort/filter/search, the contact-detail screen, "dismiss" (no longer interested) and recover-dismissed flows, address autocomplete + map pinning, share-link import (`SharedGoodNewsListener`, `contactImport`, `contactShareLink`, `lib/linking` for deep-link / Universal Link routing), `GenderIcon`, the `Archive` + `Dismiss` swipe actions, and a debug `JsonViewer`. _Contains: screens, components, hooks, lib, stores._
...
- [`mileage/`](/src/features/mileage) — **Vehicle mileage the User intends to report for reimbursement.** Vehicle and mileage-category management, direct-distance and start/finish odometer entries, per-entry history, Month / Service Year / All Time summaries, category and vehicle breakdowns, and a separate monthly text report. A summary card in Progress opens its dedicated dashboard screen; onboarding and Preferences compose its setup components. Durable records and unit/validation helpers live in shared `stores/mileage`, `types/mileage`, and `lib/mileage` for backup and iCloud consumers. _Contains: screens, components, lib._
...
- [`progress/`](/src/features/progress) — **History across time windows.** ProgressScreen composes time history with optional mileage summary cards across Month / Year / All Time. Time owns the lifetime-hours card, year-by-year breakdown, `AddEarlierYearSheet`, and `MilestoneAdjustSheet`; each mileage card opens the dedicated mileage dashboard in the corresponding period. Checkbox-mode Publishers can access mileage here when enabled. Page-level orchestrator; classified as `app`. _Contains: screens, components._
...
- [`settings/`](/src/features/settings) — **The Settings drawer and every preference surface, plus the backup/export domain.** Top-level `SettingsScreen` (drawer), `MoreScreen` (overflow menu), `ImportAndExportScreen` (backup/restore JSON), the `BackupReminder` banner (shown on Home when local export is overdue), and the nested `preferences/` screens (publisher type, goals, theme, notifications, app icon, hemisphere, locale, …). Owns the settings-only `inputs/InputRowButton` row component. Composes preferences from every other feature. Page-level orchestrator; classified as `app`. _Contains: screens, components, hooks, lib._
